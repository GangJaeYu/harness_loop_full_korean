...
- **`구현됨 (as-is)` 행은 상태를 올리지 않는다.** `code-to-docs` 가 코드에서 역으로 쓴 PRD 의 행이고, 그 기능은 이미 있다.
  담당에는 지금 동작을 붙잡는 테스트 태스크만 붙이고 구현 태스크는 만들지 않는다 — `계획됨` 으로 올리면 "만들 것"으로 읽혀 뒤 스킬이 있는 기능을 다시 만든다.
  10단계 부분 갱신에서도 같다
...
`plan_setup/` 을 쓴 뒤 프로젝트 루트의 `CLAUDE.md` 와 `AGENTS.md` 를 **같은 내용으로** 갱신한다. 사용자가 직접 연 세션과 루프의 자식 세션은 이 스킬을 읽지 않고 시작하는데,
...
- 둘 다 없으면 아래 뼈대로 만든다. 있으면 **`## 플랜 (plan_setup/)` 절과 `현재 단계` 줄만** 고친다. 다른 절과 사용자가 손으로 적은 내용은 건드리지 않는다
...
`## 플랜 (plan_setup/)` 절에는 이것만 적는다. 이 절은 루프의 자식 세션이 STATE.md 를 모른 채 코드를 고치는 것을 막는 마지막 안전망이다.

```markdown
## 플랜 (plan_setup/)
- 진실은 `plan_setup/STATE.md` 다. 세션을 열면 먼저 읽는다
- 태스크 ID = 파일 이름(`phase-NN-task-MM`). status 는 pending / in_progress / blocked / failed / completed
- 태스크 문서의 `files` 밖 파일은 고치지 않는다
- STATE.md·LOG.md·태스크 `status` 를 **누가** 쓰는지는 `loop_setup/LOOP.md` 가 정한다. 루프 전이면 이 세션이 쓴다(LOG.md 는 append-only)
- `blocked` 태스크는 건드리지 않는다. 차단 해소는 PRD 의 OQ 에서 한다
```
...
| PRD 추적표 | `담당 태스크` 와 `상태` 두 열만 갱신됐고, 나머지 열은 그대로임. `구현됨 (as-is)` 행은 담당만 붙고 상태가 그대로임 | 채운다 / 되돌린다 |
| 일정 침범 | 날짜·기간·공수 추정이 0건 | 지운다 |
| 하네스 침범 | 게이트의 실행 방법·설정 파일 내용이 0건 | 지우고 Layer 4로 넘긴다 |
| `CLAUDE.md` · `AGENTS.md` | 두 파일이 같고 `## 플랜` 절이 갱신됐고 각 200줄 이하 | 자기 절만 고친다. 넘치면 규칙을 문서로 돌려보내고 가리키는 줄만 남긴다 |
