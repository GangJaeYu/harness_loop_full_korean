...
`plan_setup/` 을 쓴 뒤 프로젝트 루트의 `CLAUDE.md` 를 갱신한다. 사용자가 직접 연 세션과 루프의 자식 세션은 이 스킬을 읽지 않고 시작하는데,
그때 자동으로 읽히는 파일은 `CLAUDE.md` 뿐이다. 그래서 **어느 세션에서든 반드시 지킬 것**만 여기에 두고 세부 규칙은 문서를 가리킨다 —
규칙을 옮겨 적으면 두 곳이 곧 어긋나고, 어긋나면 세션은 짧은 쪽을 믿는다.

- 없으면 아래 뼈대로 만든다. 있으면 **`## 플랜 (plan_setup/)` 절과 `현재 단계` 줄만** 고친다. 다른 절과 사용자가 손으로 적은 내용은 건드리지 않는다
...
| `CLAUDE.md` | `## 플랜` 절이 갱신됐고 전체 200줄 이하 | 자기 절만 고친다. 넘치면 규칙을 문서로 돌려보내고 가리키는 줄만 남긴다 |
