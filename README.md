...
브랜치나 태그를 고정하려면 `...@main` 또는 `...#v1.3.0` 을 붙입니다.
...
**모든 스킬은 산출물을 쓴 뒤 프로젝트 루트의 `CLAUDE.md` 도 함께 갱신합니다.** 스킬을 읽지 않고 시작하는 세션(직접 연 세션, 루프의 자식 세션)도
STATE.md·게이트 명령·LOOP.md 를 알고 시작하도록, 어느 세션에서든 지킬 것만 레이어별 절(제품·기술·구조·플랜·하네스·루프)에 담고 200줄을 넘기지 않습니다.
세부 규칙은 옮겨 적지 않고 해당 문서를 가리키며, 각 스킬은 자기 절만 고치고 사용자가 손으로 적은 내용은 건드리지 않습니다.
코덱스에서는 `AGENTS.md` 에 `CLAUDE.md 를 읽고 따른다.` 한 줄을 둡니다.
...
**세팅 때 묻는 것**: 병렬로 갈지(기본 예)·동시 에이전트 수·슬롯 격리 방식, 미활성 게이트를 달고 완료시켜도 되는지.

이 스킬은 루프를 **세우기만** 하고 돌리지 않습니다. 문서를 다 쓴 뒤 "첫 사이클을 시작할까요?"라고 묻고, 그 뒤로는 스킬이 아니라 `LOOP.md` 가 안내합니다.

### 4.8 `loop-update` — 루프 갱신 (Layer 5)

**언제**: 태스크가 추가되어 DAG 가 달라졌을 때, 병렬 폭·격리 방식을 바꿀 때, 루프가 같은 자리에서 반복해 멈출 때.
...
5. "루프 세팅해줘" → 병렬 여부·격리 방식·미활성 승인을 답하고 `DAG.md` 로 실행 순서를 검토합니다
