...
- **어디서 도나 — `main` 은 사용자 자리**: 루프는 `main`(저장소 기본 브랜치 — `master` 면 그 이름. 세팅 때 알아내 `LOOP.md` 에 적습니다)에서 작업하지 않습니다. 코디네이터는 자기 워크트리(`coordinator`)에서 문서(`STATE.md`·`LOG.md`·태스크 문서)를 쓰고 배정·병합을 하며, 첫 워커(`worker-1`)도 그 트리에서 돕니다. 워크트리는 **동시에 코드를 쓰는 둘째 워커부터**만 `worker-N` 으로 더 만듭니다(폭이 1이면 워크트리 0개). 검증 워커는 구현이 돈 자리에서 **이어서**(동시가 아니라 순서대로) 돕니다. 완료된 태스크는 코디네이터 트리에 병합된 뒤 `main` 에 fast-forward 되므로 `main` 은 언제나 '검증된 최신'이고, 사용자가 중간에 살펴보거나 손대도 됩니다. 오르카 화면에서는 `coordinator` 카드(코디네이터 + `worker-1`)와 `worker-2`… 카드로 보입니다
- **코디네이터가 배정**: 워커는 스스로 태스크를 고르지 않습니다. 진입 차수 0·차단 아님·충돌 없음인 태스크를 코디네이터가 폭만큼 나눠 줍니다. 문서는 코디네이터만 씁니다. 워크트리는 태스크마다가 아니라 슬롯 수(동시 상한)만큼 만들어 재사용하고, 재배정 직전에 브랜치를 되돌리고 untracked 파일을 지웁니다(`git clean -fd`). 저장소가 OneDrive 같은 동기화 폴더 아래면 형제 폴더 대신 저장소 안 `.wt/` 나 동기화 밖 경로를 씁니다
