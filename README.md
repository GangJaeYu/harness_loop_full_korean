...
예: `GangJaeYu/harness_loop_full_korean@v1.10.0` (태그) 또는 `...@main` (브랜치).
...
Layer 5  loop-setup             → loop_setup/LOOP.md · DAG.md
         loop-update              (진행에 맞춰 갱신)
              ...
              LOOP.md 가 안내하는 사이클: 태스크 병렬 구현 → 게이트 4단계 → 분리 검증 → 완료 → 다음
                                       ↓  (프로젝트 완료 뒤)
Layer 6  code-to-docs           → 문서 없는 코드 → as-is docs/ · phase-01 현행 고정 · 하네스 · 루프
         refactor-plan          → TRD·ARCHITECTURE 후미 "## 리팩토링 N" · phase-NN-refactor-<이름>
         feature-add            → IDEA·PRD·TRD·ARCHITECTURE 후미 "## 추가 N" · phase-NN-<기능>
         bug-fix                → 회귀 검사(게이트) · LOG.md "### BUG-NN" · 커밋 1회
```
...
### Layer 6 — 프로젝트 완료 뒤 (코드→문서, 리팩토링, 기능 추가, 버그 수정)

Layer 0~5 는 "문서 → 코드" 한 방향입니다. 프로젝트가 끝나면 세 가지 일이 생기고(코드 품질 올리기, 기능 더하기, 안 되는 것 고치기), 문서 없이 만든 코드를 이 흐름에 올리는 일도 있습니다. 넷 다 **기존 문서를 다시 쓰지 않습니다** — 문서 후미에 절을 붙이고(`## 추가 N`, `## 리팩토링 N`, 같은 문서 안에서는 뒤 절이 앞 절을 이깁니다), 플랜에는 끝번호 phase 를 더하며, 필요한 앞 레이어 스킬을 부분 갱신 모드로 부릅니다. **루프가 도는 중(`STATE.md` 의 Phase Overview 에 `in_progress`)에는 시작하지 않습니다.**

### 4.9 `code-to-docs` — 문서 없는 코드를 올린다 (Layer 6)

**언제**: 바이브 코딩으로 만든 코드처럼 `docs/PRD.md` 가 없는 프로젝트를 이 플러그인으로 이어 가고 싶을 때.
**무엇을 하나**: 코드를 읽고 "이렇게 만들자"가 아니라 **"지금 이렇게 되어 있다"(as-is)** 로 PRD·TRD·아키텍처를 씁니다. 코드만으로 알 수 없는 것(왜, 누가)과 버그로 보이는 동작은 판단 없이 미결(OQ)로 남깁니다. 그다음 plan-generator(phase-01 현행 고정 — 지금 동작을 붙잡는 테스트만 쓰는 phase)·harness-setup·loop-setup 을 부릅니다. 루프로 phase-01 을 끝내면 게이트가 활성이 되고, 루프로 만든 완료 프로젝트와 같은 상태가 됩니다.

### 4.10 `refactor-plan` — 리팩토링 계획 (Layer 6)

**언제**: "코드 정리하자", "라이브러리 올리자", "프레임워크 바꾸자".
**무엇을 하나**: 범위(언어·프레임워크 교체 / 라이브러리 최신화 / 코드 정리)를 묻고 코드를 읽어 고칠 목록을 만듭니다. 결정이 바뀌는 문서에만 `## 리팩토링 N` 절을 붙이고 `phase-NN-refactor-<이름>` 을 더합니다. 통과 조건은 새 기능이 아니라 **기존 게이트 전부 통과(행위 보존)** 와 구조 조건입니다.

### 4.11 `feature-add` — 기능 추가 (Layer 6)

**언제**: "xx 기능 추가하려는데", "알림 같은 걸 넣고 싶은데".
**무엇을 하나**: 요청이 모호하면 idea-brainstorm 부터, 명확하면 prd-generator 부터 부르되 각 문서에 `## 추가 N` 절만 붙입니다. 기존 요구사항이 바뀌면 "FR-003 수용 기준 2 를 다음으로 대체"로 적고 번호는 유지합니다. 끝에 `phase-NN-<기능>` 을 더하고 harness-update·loop-update 를 부릅니다.

### 4.12 `bug-fix` — 버그 수정 (Layer 6)

**언제**: 다 만든 앱을 **쓰다가** "xx 가 안 돼"를 발견했을 때. 개발 중 게이트 실패는 이 스킬이 아니라 루프의 실패 분류·재시도 3회가 다룹니다.
**무엇을 하나**: 먼저 버그인지 요구 변경인지 가릅니다 — 고친 뒤의 동작을 새 결정(누가 할 수 있나, 새 응답 코드, 새 화면) 없이 한 줄로 쓸 수 없으면 feature-add 로 돌립니다. 버그면 재현 절차와 기대 동작을 `LOG.md` 에 `### BUG-NN` 줄로 **고치기 전에** 적고, 원인을 확인하고, 고치기 전에 실패하는 회귀 검사를 게이트에 즉시 활성으로 넣고(harness-update), 고치고, 게이트 넷을 통과시킨 뒤(분리 검증은 그 재현 줄이 수용 기준) 커밋 1회(메시지에 `BUG-NN`) 합니다. 게이트가 하나라도 못 돌면 커밋하지 않고 멈춥니다. 문서·플랜·phase 는 건드리지 않습니다.
**예시**:

> 사용자: '안 끝난 것만' 체크하면 목록이 싹 비어 버려.
> 에이전트: BUG-01 로 적었습니다. 원인은 `routes/tasks.js` 21행의 문자열·불리언 비교이고, 회귀 검사가 고치기 전 2건 실패하는 것을 확인한 뒤 고쳤습니다. 게이트 넷 통과 후 `BUG-01: …` 로 커밋했습니다.
...
| ② 오르카 스크립트 구동 | 오르카 터미널 안에서 도는 구동 스크립트(`harness_setup/scripts/loop-drive.*`)가 `orca orchestration worker-start --agent claude\|codex` 로 워커를 띄우고 기다림. 판단은 라운드마다 새로 여는 코디네이터 자식(`claude -p`)이 함(6.6) | 오르카가 있을 때의 기본 |
...
- **어디서 도나 — `main` 은 사용자 자리**: 루프는 `main`(저장소 기본 브랜치 — `master` 면 그 이름. 세팅 때 알아내 `LOOP.md` 에 적습니다)에서 작업하지 않습니다. 코디네이터는 자기 워크트리(`coordinator`)에서 문서(`STATE.md`·`LOG.md`·태스크 문서)를 쓰고 배정·병합을 하며, 첫 워커(`worker-1`)도 그 트리에서 돕니다. 워커 워크트리는 **동시에 코드를 쓰는 둘째 워커부터**만 `worker-N` 으로 더 만듭니다(폭이 1이면 코디네이터 트리 하나뿐, 워커 워크트리 0개). 검증 워커는 구현이 돈 자리에서 **이어서**(동시가 아니라 순서대로) 돕니다. 완료된 태스크는 코디네이터 트리에 병합된 뒤 `main` 에 fast-forward 되므로 `main` 은 언제나 '검증된 최신'이고, 사용자가 중간에 살펴보거나 손대도 됩니다. 오르카 화면에서는 `coordinator` 카드(코디네이터 + `worker-1`)와 `worker-2`… 카드로 보입니다
- **코디네이터가 배정**: 워커는 스스로 태스크를 고르지 않습니다. 진입 차수 0·차단 아님·충돌 없음인 태스크를 코디네이터가 폭만큼 나눠 줍니다. 문서는 코디네이터만 씁니다. 워커 워크트리는 태스크마다가 아니라 슬롯 수(동시 상한)에서 코디네이터 트리의 슬롯 1 을 뺀 만큼만 만들어 재사용하고, 재배정 직전에 브랜치를 되돌리고 untracked 파일을 지웁니다(`git clean -fd`). 저장소가 OneDrive 같은 동기화 폴더 아래면 형제 폴더 대신 저장소 안 `.wt/` 나 동기화 밖 경로를 씁니다
- **격리**: 슬롯별 포트·DB 를 환경변수로 나눕니다(방식 셋 중 세팅 때 선택). 테스트가 DB 를 쓰면 슬롯별 DB 를 추천합니다 — 격리 없이 앱 게이트만 직렬화하는 방식에서도 unit 은 병렬로 돌아 공유 DB 에서 서로 오염되기 때문입니다
...
**phase 경계에서 사람이 할 일은 없습니다.** 구동 스크립트가 phase 경계를 그냥 지나가고, 판단하는 코디네이터 자식은 라운드마다 빈 컨텍스트로 새로 열립니다(1.10.0).
코디네이터를 대화형 세션으로 상주시키지 않는 이유는 비용입니다 — 기다리는 동안의 폴링마다 커진 컨텍스트를 통째로 다시 읽게 됩니다. 다음 라운드에 남길 말은 `STATE.md` 의 `알림:` 과 `LOG.md` 에만 적습니다.
①-병렬과 ②는 판단하는 쪽이 같고 워커를 띄우는 수단만 다릅니다.

**트리거는 실행 단계입니다**(1.10.0). 같은 검사가 여러 태스크에 걸쳐 실패하면(`LOG.md` 의 `- 실패:` 줄로 셉니다) 코디네이터가 매 라운드 그것을 세어 하네스를 고치거나, 스스로 못 고치면 `질문:` 으로 멈춥니다.
`알림:` 에는 처리 주체와 만료가 붙고, phase 를 열기 전에 만료된 알림과 판단 시점이 온 미결을 한 번에 모아 묻습니다. 루프가 사람을 기다리는 것은 둘뿐입니다 — `Next Action` 에 답 없는 `질문:` 이 있을 때, 그리고 모든 phase 가 끝났을 때.
프로젝트가 끝난 뒤의 리팩토링·기능 추가·버그 수정은 Layer 6 스킬(4.9~4.12)이 phase 를 더하거나 회귀 검사를 넣어 이어 갑니다.
...
7. 진행 상황은 언제나 `plan_setup/STATE.md`(지금 어디)와 `LOG.md`(무슨 일이)로 봅니다. 실시간 값은 코디네이터 워크트리의 것이고, `main` 의 것은 마지막 태스크 완료 시점입니다
...
├── loop-setup/
├── loop-update/
├── code-to-docs/           Layer 6 — 프로젝트 완료 뒤
├── refactor-plan/
├── feature-add/
└── bug-fix/
...
- **1.10.0** — 실전 프로젝트의 세션 기록을 집계해 나온 비용·정지 문제를 고쳤습니다.
  - `loop-setup`·`loop-update` — ② 오르카 구동에서 코디네이터 상주를 없앴습니다. 오르카 터미널 안의 구동 스크립트가 워커를 띄우고 기다리고, 판단은 라운드마다 새 코디네이터 자식이 합니다. 워커의 질문은 질문 답변 자식이 받아 문서로 답하거나 `질문:` 으로 올립니다
  - 반복 실패 판정을 지시문의 한 단계로 넣고(`- 실패:` 줄 형식), `알림:` 에 주인과 만료를, phase 를 열 때의 확인을 더했습니다. 이전 `LOOP.md` 는 `loop-update` 가 옮깁니다
  - 테스트가 DB 를 쓰면 슬롯별 DB 격리를 추천합니다. 워커 트리 청소를 `git reset --hard coordinator` 로 고쳤습니다
  - `harness-setup`·`harness-update` — e2e 흔들림 규칙을 출처가 붙은 표로 두고 안정화할 때마다 한 줄씩 늘립니다. `HARNESS.md` 에 공유 DB 를 쓰는 게이트와 슬롯 변수를 적습니다
- **1.9.0** — Layer 6(프로젝트 완료 뒤) 스킬 넷을 한 번에 더했습니다. 1.6.0~1.8.0 은 개발 저장소에서만 거친 중간 단계라 이 저장소에는 따로 없습니다.
  - `code-to-docs`(개발 1.6.0) — 문서 없는 코드베이스를 as-is 문서·phase-01 현행 고정·하네스·루프로 올립니다
  - `refactor-plan`(개발 1.7.0) — 범위 셋·고칠 목록·`## 리팩토링 N` 후미 절·`phase-NN-refactor-<이름>`
  - `feature-add`(개발 1.8.0) — 모호/명확 판정 뒤 앞 스킬을 후미 절 모드로 불러 `## 추가 N` 절과 `phase-NN-<기능>`
  - `bug-fix`(개발 1.9.0) — 버그/요구 변경 판정, `### BUG-NN` 재현 줄 먼저, 즉시 활성 회귀 검사, 게이트 넷 통과 뒤 커밋 1회
  - `plan-generator` — as-is 문서의 `구현됨 (as-is)` 행은 담당 태스크가 붙어도 상태를 `계획됨` 으로 올리지 않습니다(code-to-docs 용)
- **1.5.3 이하** — 태그와 커밋 메시지를 보세요
