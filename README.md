...
예: `GangJaeYu/harness_loop_full_korean@v1.9.0` (태그) 또는 `...@main` (브랜치).
...
| ② 오르카 오케스트레이션 | `orca orchestration worker-start --agent claude\|codex` 로 워커를 띄움. phase 가 끝나면 코디네이터가 다음 phase 코디네이터를 새 터미널로 띄움(6.6) | 오르카가 있을 때의 기본 |
...
**phase 경계에서 사람이 할 일은 없습니다.** 코디네이터(②)는 phase 하나가 수명입니다 — 컨텍스트가 쌓이지 않게 phase 완료 판정 뒤 세션을 닫는데,
닫기 직전에 **다음 phase 코디네이터를 새 터미널(새 프로세스)로 스스로 띄웁니다**(1.5.3). 새 코디네이터에 넘기는 것은 시작 프롬프트의 `phase-NN` 한 값뿐이고,
앞 phase 의 대화·요약·권고는 넘기지 않습니다 — 다음 세션에 남길 말은 `STATE.md` 의 `알림:` 과 `LOG.md` 에만 적고, 새 세션은 그 문서를 읽고 시작합니다.
①-병렬은 스크립트가 같은 일을 합니다. 루프가 사람을 기다리는 것은 둘뿐입니다 — `Next Action` 에 답 없는 `질문:` 이 있을 때, 그리고 모든 phase 가 끝났을 때.
