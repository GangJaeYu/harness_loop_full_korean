...
예: `GangJaeYu/harness_loop_full_korean@v1.5.0` (태그) 또는 `...@main` (브랜치).
...
- **어디서 도나 — `main` 은 사용자 자리**: 루프는 `main` 에서 작업하지 않습니다. 코디네이터는 자기 워크트리(`coordinator`)에서 문서(`STATE.md`·`LOG.md`·태스크 문서)를 쓰고 배정·병합을 하며, 워커는 슬롯마다 하나씩인 `worker-N` 워크트리에서 돕니다(폭이 1이면 `worker-1` 하나). 검증 워커는 구현이 돈 `worker-N` 에서 **이어서**(동시가 아니라 순서대로) 돕니다. 완료된 태스크는 코디네이터 트리에 병합된 뒤 `main` 에 fast-forward 되므로 `main` 은 언제나 '검증된 최신'이고, 사용자가 중간에 살펴보거나 손대도 됩니다. 오르카 화면에서는 `coordinator`·`worker-1`·`worker-2` 카드로 보입니다
